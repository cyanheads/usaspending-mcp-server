...
Thanks for using `usaspending-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

- [Report a bug](https://github.com/cyanheads/usaspending-mcp-server/issues/new?template=bug_report.yml)
- [Request a feature](https://github.com/cyanheads/usaspending-mcp-server/issues/new?template=feature_request.yml)
- [Float an idea or ask a question](https://github.com/cyanheads/usaspending-mcp-server/issues/new) — free-form, no template

Both forms are structured, and filling in the fields is what makes an issue actionable. Issues are the contribution path here: code changes go through the maintainer's own workflows, so a precise report with a reproduction is the fastest route to a fix.

## Server bug or framework bug?

`usaspending-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong data, a USAspending.gov API call fails, a schema doesn't match reality, a description misleads the model.
...
Don't open a public issue for a vulnerability. See [SECURITY.md](./SECURITY.md) — GitHub's **Security** tab → **Report a vulnerability**, or email **security@caseyjhand.com**.
